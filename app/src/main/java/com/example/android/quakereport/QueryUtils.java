...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes(String usgs_url) {
        ...
        try {
            ...
            JSONObject sample_json_object=new JSONObject(json_response);
            JSONArray basejsonarray=sample_json_object.getJSONArray("features");
            for(int i=0;i<basejsonarray.length();i++)
            {
                JSONObject currentEarthquake=basejsonarray.getJSONObject(i);
                JSONObject properties=currentEarthquake.getJSONObject("properties");
                Double magnitude=properties.getDouble("mag");
                String location=properties.getString("place");
                String time=properties.getString("time");
                String url_json=properties.getString("url");
                long t=Long.parseLong(time);
                earthquakes.add(new Earthquake(magnitude,location,t,url_json));
            }
    ...
    private static URL createUrl(String usgs_url)
    {
        ...
        try {
            ...
            Log.e("LOG_ERROR","Error with creting url",e);
        }
        return url;
    }
